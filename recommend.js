...
function splitChanceOutcomesFromLines(lines) {
  ...
  for (const raw of (lines || [])) {
    ...
    if (isEither(line) || isOr(line)) {
      ...
      if (!current) { // no header yet -> not a chance pattern
        return null;
      }
...
export function parseRewardLine(line) {
  ...
  if (statPair) {
    ...
    const key =
      keyLc === "sp" || keyLc === "speed" ? "speed" :
      keyLc === "sta" || keyLc === "stamina" ? "stamina" :
      keyLc === "pwr" || keyLc === "power" ? "power" :
      keyLc === "guts" ? "guts" :
      keyLc === "int" || keyLc === "intelligence" || keyLc === "wit" || keyLc === "wisdom" ? "intelligence" :
      keyLc;
...
function scoreItem(item) {
  switch (item.type) {
    case "energy":        return { score: WEIGHTS.energy * (item.value || 0), note: `Energy ${fmtPlus(item.value)}` };
    case "stat": {
      const mult = STAT_MULT[item.key] ?? 1.0;
      const s = WEIGHTS.stat * mult * item.value;
      return { score: s, note: `${cap(item.key)} ${fmtPlus(item.value)} (${mult.toFixed(2)}×)` };
    }
    case "bond":          return { score: WEIGHTS.bond * item.value, note: `Bond ${fmtPlus(item.value)}` };
    case "skill_points":  return { score: 0.25 * (item.value || 0), note: `Skill points ${fmtPlus(item.value)}` };
    case "mood":          return { score: WEIGHTS.mood * item.value, note: `Mood ${item.value > 0 ? "Up" : "Down"}` };
    case "hint":          return { score: WEIGHTS.hint * (item.value || 1), note: `Hint: ${item.name} ${fmtPlus(item.value || 1)}` };
    case "status_gain": {
      const name = normStatusName(item.text);
      const custom = STATUS_WEIGHTS[name];
      const s = (custom !== undefined) ? custom : WEIGHTS.status_gain;
      const note = (custom !== undefined) ? `Status gained: ${name}` : `Status gained: ${name || item.text}`;
      return { score: s, note };
    }
    case "status_loss": {
      const name = normStatusName(item.text);
      ...
      const note = (custom !== undefined) ? `Status removed: ${name}` : `Status removed: ${name || item.text}`;
      return { score: s, note };
    }
    default:              return { score: WEIGHTS.text, note: item.text || item.raw || "" };
  }
}
...
export function scoreOption(groups) {
  ...
  function scoreLineRaw(s) {
    ...
    if (m) {
      ...
      const mult =
        key === 'guts' ? 0.6 :
        key === 'wisdom' || key === 'intelligence' ? 0.8 : // matches your file’s STAT_MULT
        1.0;
      return 1.0 * mult * val;
    }
  ...
  const isStatLine = (ln) => /^(Speed|Power|Stamina|Guts|Wisdom|Intelligence)\s*[+\-]?\d+/i.test(ln);
...
function sumEnergyEntry(entry) {
  let s = 0;
  if (entry?.parsed) {
    entry.parsed.flat().forEach(it => { if (it && it.type === "energy") s += (it.value || 0); });
    return s;
  }
...
function sumHintsEntry(entry) {
  let s = 0;
  if (entry?.parsed) {
    entry.parsed.flat().forEach(it => { if (it && it.type === "hint") s += (it.value || 1); });
    return s;
  }
  (entry?.groups || []).forEach(lines => {
    (lines || []).forEach(line => {
...
export function chooseRecommendedOption(eventData) {
  ...
  if (!eventData || !eventData.options || typeof eventData.options !== "object") {
    return { label: null, score: 0, byLabel: {}, notes: ["No options to evaluate."] };
  }

  const byLabel = {};
  for (const [label, groups] of Object.entries(eventData.options)) {
    const { total, breakdown, parsed, groups: kept } = scoreOption(groups);
    byLabel[label] = { score: total, breakdown, parsed, groups: kept || groups };
  }
  ...
  const numAtMax = labels.reduce((n, l) => n + (Math.abs(byLabel[l].score - maxScore) < EPS ? 1 : 0), 0);
  let best = null;
  ...
  if (numAtMax === 1) {
    ...
    let bestEnergy = -Infinity, energyWinners = [];
    candidates.forEach(l => {
      const e = sumEnergyEntry(byLabel[l]);
      if (e > bestEnergy) { bestEnergy = e; energyWinners = [l]; }
      else if (e === bestEnergy) { energyWinners.push(l); }
    });

    if (energyWinners.length === 1) {
      best = energyWinners[0];
    } else {
      let bestHints = -Infinity, hintWinners = [];
      energyWinners.forEach(l => {
        const h = sumHintsEntry(byLabel[l]);
        if (h > bestHints) { bestHints = h; hintWinners = [l]; }
        else if (h === bestHints) { hintWinners.push(l); }
      });
